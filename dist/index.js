...
var CronosZkEVMEnvSchema = z.object({
  CRONOSZKEVM_ADDRESS: z.string().min(1, "Cronos zkEVM address is required").refine((address) => isAddress(address, { strict: false }), {
    message: "Cronos zkEVM address must be a valid address"
  }),
  CRONOSZKEVM_PRIVATE_KEY: z.string().min(1, "Cronos zkEVM private key is required").refine((key) => /^[a-fA-F0-9]{64}$/.test(key), {
    message: "Cronos zkEVM private key must be a 64-character hexadecimal string (32 bytes) without the '0x' prefix"
  })
});
...
import { mainnet, cronoszkEVM as cronoszkEVM2 } from "viem/chains";

// src/constants/index.ts
var ZKCRO_ADDRESS = "0x000000000000000000000000000000000000800A";
...
var TransferAction = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    if (!isAddress3(content.recipient, { strict: false })) {
      ...
      try {
        ...
        if (isAddress3(resolvedAddress, { strict: false })) {
    ...
    try {
      ...
      let hash;
      if (content.tokenAddress.toLowerCase() !== ZKCRO_ADDRESS.toLowerCase()) {
        const tokenInfo = ERC20_OVERRIDE_INFO[content.tokenAddress.toLowerCase()];
        const decimals = (tokenInfo == null ? void 0 : tokenInfo.decimals) ?? 18;
